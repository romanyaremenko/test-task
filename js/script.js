...
function SendForm () {
  ...
  formData.append('gender', allAnswers[0].querySelector('p').innerHTML);
  formData.append('birthday', allAnswers[1].querySelector('p').innerHTML);
  formData.append('allergy', allAnswers[2].querySelector('p').innerHTML);


  var request = new XMLHttpRequest();
  request.open('POST', './backend/contactUs.php');
  request.addEventListener('readystatechange', function () {
    // если запрос завершился и код ответа сервера OK (200), то
    if (this.readyState == 4 && this.status == 200) {
      document.querySelector(".buttons").style.display = "none"
      document.getElementById('success').innerHTML = "Thank you! Your message has been successfully sent.";
    }
  });
...
document.querySelector('.submit-roulette').addEventListener('click',function (e) {
  e.preventDefault();
  SendForm()
})
